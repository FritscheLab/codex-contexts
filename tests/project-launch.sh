...
source "$LAUNCH_TEST_REPO/bin/codex-home"
...
quiet_check "$LAUNCH_TEST_REPO/bin/codex-home" vscode-project "$project" >/dev/null
...
expect_rejection "project selects 'launch-unit', not 'launch-other'" \
  "$LAUNCH_TEST_REPO/bin/codex-home" vscode launch-other "$project"
...
approve_fixture
expect_rejection "but CODEX_HOME is" "$LAUNCH_TEST_REPO/bin/codex-home" vscode-project "$project"
write_fixture launch-unit "$LAUNCH_TEST_ROOT/nonexistent-home"
approve_fixture
expect_rejection "CODEX_HOME does not exist" "$LAUNCH_TEST_REPO/bin/codex-home" vscode-project "$project"
printf 'export CODEX_IDENTITY=launch-unit\n' >"$project/.envrc"
approve_fixture
expect_rejection "did not load a Codex identity" "$LAUNCH_TEST_REPO/bin/codex-home" vscode-project "$project"
...
expect_rejection "is blocked" "$LAUNCH_TEST_REPO/bin/codex-home" vscode-project "$project"
mv "$project/.envrc" "$project/.envrc.saved"
expect_rejection "cannot read" "$LAUNCH_TEST_REPO/bin/codex-home" vscode-project "$project"
...
for launch_command in vscode-project vscode; do
  ...
  if [[ "$launch_command" == vscode ]]; then
    quiet_check "$LAUNCH_TEST_REPO/bin/codex-home" vscode launch-unit "$project" >/dev/null
  else
    quiet_check "$LAUNCH_TEST_REPO/bin/codex-home" vscode-project "$project" >/dev/null
  fi
...
quiet_check env CODEX_IDENTITY=default CODEX_HOME="$default_home" \
  "$LAUNCH_TEST_REPO/bin/codex-home" _vscode-loaded "$project" default \
...
approve_fixture
launch_status=0
CODEX_LAUNCH_TEST_EXIT=37 "$LAUNCH_TEST_REPO/bin/codex-home" vscode-project "$project" \
